...
# Dynamically fallback to an in-memory database if running in the cloud to prevent ephemeral filesystem wipes
...
class ApplicantProfile(BaseModel):
    ...
    # FIXED: Replaced empty 'examples=' block with a valid baseline integer example format
...
def evaluate_credit_risk(profile: ApplicantProfile, records: List[MpesaRecord]) -> dict:
    ...
    return {
        ...
        "metrics": {
            ...
            "concurrent_lenders_count": lender_count
        },
        "risk_flags": flags,
